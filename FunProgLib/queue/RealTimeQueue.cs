...
public static class RealTimeQueue<T>
{
    ...
    private static Lazy<Stream<T>.StreamCell> Rotate(Lazy<Stream<T>.StreamCell> xp, FunList<T>.Node yp, Lazy<Stream<T>.StreamCell> a)
    {
    ...
    private static Queue Exec(Lazy<Stream<T>.StreamCell> f, FunList<T>.Node r, Lazy<Stream<T>.StreamCell> sp)
    {
        if (sp.Value != null) return new Queue(f, r, sp.Value.Next);
        var fp = Rotate(f, r, Stream<T>.DollarNil);
        return new Queue(fp, FunList<T>.Empty, fp);
    }

    public static Queue Snoc(Queue q, T x) => Exec(q.F, FunList<T>.Cons(x, q.R), q.S);
