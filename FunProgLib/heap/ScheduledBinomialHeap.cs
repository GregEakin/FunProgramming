...
public static class ScheduledBinomialHeap<T> where T : IComparable<T>
{
    ...
    private static Tree Link(Tree t1, Tree t2)
    {
        if (t1.Node.CompareTo(t2.Node) <= 0)
            return new Tree(t1.Node, FunList<Tree>.Cons(t2, t1.TreeList));
        return new Tree(t2.Node, FunList<Tree>.Cons(t1, t2.TreeList));
    }
    ...
    private static FunList<Lazy<Stream<Digit>.StreamCell>>.Node Exec(FunList<Lazy<Stream<Digit>.StreamCell>>.Node list)
    {
        if (list == null) return null;
        if (list.Element.Value != null && list.Element.Value.Element == Zero) return FunList<Lazy<Stream<Digit>.StreamCell>>.Cons(list.Element.Value.Next, list.Next);
        return list.Next;
    }
    ...
    public static Heap Insert(T x, Heap heap)
    {
        ...
        var list = FunList<Lazy<Stream<Digit>.StreamCell>>.Cons(dsp, heap.Schedule.DigitStreamList);
    ...
    private static Lazy<Stream<Digit>.StreamCell> OneMap(FunList<Tree>.Node list)
    {
    ...
    {
        var stuff = RemoveMinTree(heap.DigitStream);
        var reverse = FunList<Tree>.Reverse(stuff.Tree.TreeList);
